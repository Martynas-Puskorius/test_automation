...
public class LoginItem_Create_Edit_Delete extends Base{
    ...
    public void CreateLoginItemAccountless() throws Exception {
        ...
        driver.hideKeyboard();
        File something = ((TakesScreenshot) driver).getScreenshotAs(OutputType.FILE);
        ...
        addLogin.getItemSettingsMore().click();
        //driver.findElements(By.xpath("//*[@text='Move to trash']"));
        Buttons buttons = new Buttons(driver);
        buttons.getMoveToTrash().click();

        doVisualCheck("Delete login");

        homeScreen.getBrowse().click();

        Browse browse = new Browse(driver);
        browse.getAllTrash().click();
        doVisualCheck("Deleted login");
